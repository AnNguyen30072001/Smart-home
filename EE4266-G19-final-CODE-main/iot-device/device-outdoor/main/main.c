...
#define COAP_OBSERVE_URI "coap://172.20.10.2:5683/obs" 
#define COAP_POST_URI "coap://172.20.10.2:5683/POST"
#define BLINK_GPIO GPIO_NUM_2
#define DHT11_PIN 4
#define NODE_ID 104
#define DHT_PERIOD 3000
#define TIME_PUMP 20000
#define NUM_TIMERS 5
...
static void coap_client_post(void *p)
{
    ...
    while (1) {
        ...
        if (dht_read_data(DHT_TYPE_DHT11, (gpio_num_t)DHT11_PIN, &humidity, &temperature) == ESP_OK)
        {
            ...
            // check_alarm();
            length = sprintf(buf, "{\"ID\":%d,\n\"temp\":%d,\n\"humi\":%d,\n\"state\":\"%s\"}", NODE_ID, temperature/10, humidity/10,(state == true) ? "on": "off");
        ...
        vTaskDelay(DHT_PERIOD / portTICK_PERIOD_MS);
